...
num_of_bins=$(cat "input_info.json" | python3 -c "import sys, json; print(json.load(sys.stdin)['num_of_bins'])")
least_num_of_datapoints=$(cat "input_info.json" | python3 -c "import sys, json; print(json.load(sys.stdin)['least_num_of_datapoints_in_each_bin'])")
target_variable_column=$(cat "input_info.json" | python3 -c "import sys, json; print(json.load(sys.stdin)['target_variable_column'])")
csv_file_name=$(cat "input_info.json" | python3 -c "import sys, json; print(json.load(sys.stdin)['csv_file_name'])")
