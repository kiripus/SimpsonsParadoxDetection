...
int main(int argc, char *argv[]) {
    ...
    for (int j = 0; j < N_columns; j++) // for each feature
    {
        ...
        for (int nbins_feature = 2;; nbins_feature++) {
            ...
            if (lambda_stopping == true) // lambda optimization
                if (lambda_stopping_condition(DL, best_R2_improvement, R2_improvements, splits, R2_improvement)) {
                    cout << "     - Loss function has reached minimum (neglect last " << DL_thresh
                         << " splits); breaking" << endl;
                    break;
                }
            ...
            if (mindatapoints_stopping == true) // min number of datapoints optimization
            {
                ...
                if (mindatapoints_feature < mindatapoints) {
                    ...
                    cout << "     - mindatapoints: " << mindatapoints_feature << " datatpoints in next split (< "
                         << mindatapoints << "); breaking" << endl;
                    break;
                }
            }
            // output the information
            cout << "     - [" << pl << "," << pu << "] split into [" << pl << "," << split << "] and [" << pl2 << ","
                 << pu << "], ";
            ...
            for (auto it = std::next(deltaR2_p.begin()); it != deltaR2_p.end(); ++it) {
                if (it->second > best_R2_improvement) {
                    ...
                    pl = *std::next(range.find(std::prev(
                            it)->first)); //  the bottom of the this interval is the next element after the end of the last interval
                    pu = it->first;
                }
            }
        ...
        */
        ...
        //  - i) save the splits
        if (j + 1 == N_columns) {
            features[j].pop_back(); // delete the endl character from the last input feature
    ...
    clock_t end = clock();
    cout << "\nBINNING COMPLETE\nTotal procedure time = " << 1.0 * (end - start) / CLOCKS_PER_SEC << " seconds."
         << endl;
